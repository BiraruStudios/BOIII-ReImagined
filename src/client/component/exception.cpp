    ...
    namespace {
        ...
        void display_error_dialog() {
            ...
            std::map<std::pair<DWORD, std::size_t>, std::string> known_errors = {
                {
                    {EXCEPTION_ACCESS_VIOLATION, 0x142276E79},
                    "Restarting your system is the only known way to temporarily fix this."
                }
            };

            std::string error_str = utils::string::va(
            ...
            game::show_error(error_str.data(), "BOIII ReImagined ERROR");
        ...
        void reset_state() {
            if (is_recoverable()) {
                ...
                game::Com_Error(game::ERR_DROP, "Fatal error (0x%08X) at 0x%p (0x%p).\nA minidump has been written.\n\n"
                                "BOIII ReImagined has tried to recover your game, but it might not run stable anymore.\n\n"
...
#pragma warning(pop)
        ...
        void write_minidump(const LPEXCEPTION_POINTERS exceptioninfo) {
            ...
            zip_file.write(crash_name, "BOIII ReImagined Crash Dump");
        }
