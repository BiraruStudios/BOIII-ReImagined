...
namespace extension
{
    ...
    BOOL WINAPI Terminate_BOIII_Reimagined(HANDLE hProcess, UINT uExitCode)
    {
    ...
    VOID WINAPI Exit_BOIII_Reimagined(UINT uExitCode)
    {
        PVOID caller = _ReturnAddress();
        HMODULE hExtDll = GetModuleHandle(TEXT("ext.dll"));
    ...
    struct component final : generic_component
	{
		component()
		{
            ...
            try
            {
                MH_CreateHook(g_pTerminateProcess, Terminate_BOIII_Reimagined, reinterpret_cast<LPVOID*>(&pOrigTerminateProcess));
                MH_EnableHook(reinterpret_cast<LPVOID>(g_pTerminateProcess));

                MH_CreateHook(g_pExitProcess, Exit_BOIII_Reimagined, reinterpret_cast<LPVOID*>(&pOrigExitProcess));
